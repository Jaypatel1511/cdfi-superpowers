...
| credit-memo | credit-memo | `creditmemo` | 0.2.2 | Generate IC credit memos from structured deal inputs — CDFI loans, NMTC deals, impact investments. |
...
| fair-lending-screener | fair-lending-screener | `fair_lending_screener`, `fairlendingscreener` | 0.2.2 | Statistical disparate-impact analysis for HMDA data — examiner methodology, open-sourced. **Inferential fair-lending; deliberately NOT wrapped by any skill in this plugin (see caveats-and-limits.md).** ⚠️ **0.1.1 is yanked** — release-process drift shipped a breaking API change in a patch release; superseded by 0.2.0. |
